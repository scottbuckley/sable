...
public:
  ...
  UCBWalker * step(const bdd & letter) {
    if (has_failed) return this;
    has_failed = try_step(letter);
    return this;
  }

  UCBWalker * step(const bdd & letter, Stopwatch * timer) {
    if (has_failed) return this;
    timer->start();
      has_failed = try_step(letter);
    timer->stop();
  ...
  UCBWalker * walk(finite_word_ptr word) {
    if (!has_failed && try_walk(word))
      has_failed = true;
    return this;
  }

  UCBWalker * walk(finite_word_ptr word, Stopwatch * timer) {
    timer->start();
    if (!has_failed && try_walk(word))
      has_failed = true;
    timer->stop();
  ...
  walk_position get_position() {
    ...
    return walk_position(cur_states);
    // return cur_states;
  }
